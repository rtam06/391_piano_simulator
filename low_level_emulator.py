...
# ── Finger spacing ─────────────────────────────────────────────────────────────
# Change these to adjust physical finger spacing (in semitones).
LH_FINGER_OFFSETS = [0, 1, 3, 4, 7, 8]  # left hand: irregular white-key spacing
RH_FINGER_SPACING = 1  # right hand: half steps between fingers

def fingers_to_keys(base_key, finger_byte, hand):
    """
    base_key   : key index where finger 1 (leftmost) sits.
    finger_byte: 6-bit value, bit5(MSB)=finger1 ... bit0=finger6.
    hand       : 0=left (uses LH_FINGER_OFFSETS), 1=right (RH_FINGER_SPACING).
    Returns (active_keys, all_keys).
    """
    active_keys, all_keys = [], []
    for f in range(6):
        if hand == 0:
            k = base_key + LH_FINGER_OFFSETS[f]
        else:
            k = base_key + f * RH_FINGER_SPACING
...
def parse_commands(filepath):
    """
    New instruction set — each line: cmd_type, elapsed_ms, hand, data, 0, 0
      cmd_type 0 = Move hand:     data = key index for leftmost finger (0-53)
      cmd_type 1 = Change fingers: data = 6-bit finger byte (e.g. 63 = all down)
      hand: 0 = left, 1 = right
      elapsed_ms: absolute time in ms from start of playback
    """
    commands = []
    with open(filepath) as f:
        for line in f:
            ...
            try:
                ...
                continue
    # Sort by time so we can fire them in order
    commands.sort(key=lambda c: c[1])
    return commands
...
class PianoEmulator:
    ...
    # Not used for timing (timing is now absolute ms) but shown in the HUD
    # and used to size the falling note bars visually.
    DISPLAY_BPM = 100
    ...
    def __init__(self, filepath):
        ...
        # Per-hand state
        # hand_pos[h]     : current base key (leftmost finger position)
        # hand_fingers[h] : current finger byte
        # active_keys     : key_index -> 'left'|'right'|'both'  (for drawing)
        self.hand_pos     = {0: 0,  1: 24}   # sensible defaults
        self.hand_fingers = {0: 0,  1: 0}
        self.active_keys  = {}

        self.particles    = []
        self.falling_notes = []

        # hand_state keeps all_keys list for the overlay bracket
        self.hand_state = {0: None, 1: None}

        self.playing    = False
        self.play_thread = None
        self.cmd_index  = 0          # tracks progress for the progress bar
        self.done       = False
        self.elapsed_ms = 0          # ms since playback started (for HUD)
    ...
    # ── Sound duration estimate based on display BPM ───────────────────────────
    def _note_sound_duration(self):
        """Rough note sustain in seconds — just used for audio synthesis length."""
        quarter_s = 60.0 / self.DISPLAY_BPM
        return max(0.3, quarter_s * 1.5)

    # ── Apply a single command to emulator state ───────────────────────────────
    def _apply_command(self, cmd):
        cmd_type, elapsed_ms, hand, data = cmd
        color_side = 'left' if hand == 0 else 'right'

        if cmd_type == 0:
            # ── Move hand: data = new base key (leftmost finger position) ──
            self.hand_pos[hand] = data
            # Recompute all_keys with current finger state at new position
            _, all_keys = fingers_to_keys(data, self.hand_fingers[hand], hand)
            self.hand_state[hand] = {
                'base_key': data,
                'all_keys': all_keys,
                'is_rest':  self.hand_fingers[hand] == 0
            }
            self.current_cmd_display = (
                f"{'L' if hand==0 else 'R'} MOVE  -> key {data}  @ {elapsed_ms}ms"
            )

        elif cmd_type == 1:
            # ── Fingers changed: data = new finger byte ──
            finger_byte = data
            self.hand_fingers[hand] = finger_byte
            base_key = self.hand_pos[hand]

            active, all_keys = fingers_to_keys(base_key, finger_byte, hand)

            self.hand_state[hand] = {
                'base_key': base_key,
                'all_keys': all_keys,
                'is_rest':  finger_byte == 0
            }

            # Clear this hand's keys then re-add active ones
                ...
                if kl:
                    ...
                    for _ in range(8):
                        self.particles.append(
                            Particle(cx, cy,
                                     pcolors[np.random.randint(len(pcolors))],
                                     kl['x'], kl['w']))
                    self.falling_notes.append(
                        FallingNote(kl['x'] + 1, kl['w'], kl['is_black'],
                                    hcolor, int(self._note_sound_duration() * FPS)))

                    sound = get_sound(key_to_midi(k), self._note_sound_duration())
                    sound.play()

            if finger_byte == 0:
                self.status_msg = f"{'L' if hand==0 else 'R'} fingers lifted @ {elapsed_ms}ms"
            else:
                self.status_msg = (f"{'L' if hand==0 else 'R'} fingers "
                                   f"{bin(finger_byte)[2:].zfill(6)} @ {elapsed_ms}ms")

            self.current_cmd_display = (
                f"{'L' if hand==0 else 'R'} FINGERS {bin(finger_byte)[2:].zfill(6)} "
                f"base={base_key}  @ {elapsed_ms}ms"
            )
    ...
    def play_sequence(self):
        ...
        for idx, cmd in enumerate(self.commands):
            ...
            self.cmd_index  = idx
            target_ms       = cmd[1]          # absolute ms timestamp

            # Sleep until the command's scheduled time
            while self.playing:
                now_ms = (time.time() - t_start) * 1000.0
                self.elapsed_ms = now_ms
                remaining = target_ms - now_ms
                if remaining <= 0:
                    break
                time.sleep(min(remaining / 1000.0, 0.002))  # wake up at most every 2ms
        ...
        # Playback finished — lift all fingers
        if self.playing:
            self.playing = False
    ...
    def draw_hud(self):
        # Title
        title = self.font_title.render("Low Level File Tester", True, (220, 200, 150))
        ...
        time_surf = self.font_large.render(f"t = {elapsed_s:.2f}s", True, (200, 180, 100))
        self.screen.blit(time_surf, (30, 20))

        # BPM label (display only, not used for timing)
        bpm_surf = self.font_small.render(
            f"Display BPM: {PianoEmulator.DISPLAY_BPM}  |  "
f"LH offsets: {LH_FINGER_OFFSETS}  RH spacing: {RH_FINGER_SPACING} st",
            True, (120, 120, 140))
        self.screen.blit(bpm_surf, (30, 52))

        # Status
        status_surf = self.font_med.render(self.status_msg, True, (160, 200, 160))
        self.screen.blit(status_surf, (30, 72))

        # Current command
        cmd_surf = self.font_small.render(self.current_cmd_display, True, (100, 140, 180))
        self.screen.blit(cmd_surf, (30, 95))

        # Controls legend
        controls = [("SPACE", "Play / Restart"), ("S", "Stop"), ("ESC", "Quit")]
        cx = SCREEN_W - 200
        ...
        # Progress bar — based on command index
        if self.commands:
            progress = self.cmd_index / max(1, len(self.commands) - 1)
